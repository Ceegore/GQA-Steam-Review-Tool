...
Mirrors the layout of the Steam API tab (sections, filter grid,
action bar, log) so the two tabs feel like siblings. Cross-cutting
handlers live in ``ui/_tab_actions.py``; the action-bar widget grid
lives in ``ui/_pw_action_bar.py``.
...
from ..ui._pw_sections import (
    PwFilterRefs, build_pw_filters_section,
)
...
class PlaywrightTabController:
    ...
    def __init__(
        ...
        *,
        ...
        self._scrape_btn: Optional[ctk.CTkButton] = None
        self._export_btn: Optional[ctk.CTkButton] = None
        self._resume_btn: Optional[ctk.CTkButton] = None
        self._fetch_new_btn: Optional[ctk.CTkButton] = None
    ...
    def _build(self) -> None:
        ...
        self.filter_refs = build_pw_filters_section(self.parent)
        self._since = build_since_section(
            self.parent, prefix="pw_", log_fn=self._log,
        )
    ...
    def _build_game_section(self) -> None:
        sec = ctk.CTkFrame(self.parent)
        ...
        self._app_id_entry.pack(side="left", padx=4, fill="x", expand=True)
        self._app_id_entry.bind("<Return>", lambda _e: self._on_load())
        ctk.CTkButton(
            row, text="Load Game  ⏎", command=self._on_load, width=120,
        ...
        self._game_label.pack(side="left", padx=4, fill="x", expand=True)

        df_row = ctk.CTkFrame(sec, fg_color="transparent")
        df_row.pack(fill="x", padx=8, pady=(2, 6))
        self._dump_label = ctk.CTkLabel(
            df_row, text=f"📂 {self.dump_ctrl.dump_root}", anchor="w",
            text_color="gray",
        )
        self._dump_label.pack(side="left", padx=4, fill="x", expand=True)
        self._seen_label = ctk.CTkLabel(
            df_row, text="Already exported: 0 reviews", anchor="e",
            text_color="gray",
        )
        self._seen_label.pack(side="right", padx=4)
        ctk.CTkButton(
        ...
        ctk.CTkButton(
            ...
            command=self._actions.open_dump_folder,
        ).pack(side="right", padx=2)

    def _build_dependencies_section(self) -> None:
        sec_dep = ctk.CTkFrame(self.parent)
        sec_dep.pack(fill="x", padx=8, pady=(4, 4))
        ctk.CTkLabel(
            sec_dep, text="Dependencies", font=("", 12, "bold"),
        ).pack(anchor="w", padx=8, pady=(6, 2))
        dgrid = ctk.CTkFrame(sec_dep, fg_color="transparent")
        dgrid.pack(fill="x", padx=8, pady=(0, 6))
        ctk.CTkLabel(dgrid, text="playwright pkg:", width=140, anchor="e").grid(
            row=0, column=0, sticky="e", padx=(10, 4), pady=5,
        )
        self._pkg_status = ctk.CTkLabel(
            dgrid, text="checking…", anchor="w", text_color="gray",
        )
        self._pkg_status.grid(row=0, column=1, sticky="w", padx=4, pady=5)
        ctk.CTkButton(
            dgrid, text="Install", command=self._on_install_pkg, width=100,
        ).grid(row=0, column=2, padx=4, pady=5)
        ctk.CTkButton(
            dgrid, text="Open cache", command=self._on_open_cache, width=120,
        ).grid(row=0, column=3, padx=4, pady=5)

        ctk.CTkLabel(
            dgrid, text="chromium:", width=140, anchor="e",
        ).grid(row=1, column=0, sticky="e", padx=(10, 4), pady=5)
        self._chrome_status = ctk.CTkLabel(
            dgrid, text="—", anchor="w", text_color="gray",
        )
        self._chrome_status.grid(row=1, column=1, sticky="w", padx=4, pady=5)
        ctk.CTkButton(
            dgrid, text="Install", command=self._on_install_chrome, width=100,
    ...
    def _on_scrape(self) -> None:
        ...
        # The real Phase-7 scraper does the heavy lifting asynchronously.
        self.pw_wf.scrape(
            self.master.app_id,
